...
def input_date(call: CallbackQuery) -> None:
    ...
    if action == "DAY":
        ...
        now_year, now_month, now_day = (
            datetime.datetime.now().strftime("%Y.%m.%d").split(".")
        )
            ...
            if "checkInDate" in data:
                ...
                if int(select_date) > checkin:
                    ...
                    if data["sort"] == "DISTANCE":
                        bot.set_state(
                            call.message.chat.id, User_input_state.land_mark_in
                        )
                        bot.send_message(
                            call.message.chat.id,
                            "Теперь определимся на каком расстоянии от центра искать отели.\n"
                    ...
                    bot.send_message(
                        call.message.chat.id,
                        "Неправильный ввод! <b><u>Дата выезда должна быть больше даты заезда!</u></b>"
                ...
                if int(select_date) >= int(now):
                    ...
                    bot.send_message(
                        call.message.chat.id,
                        "Неправильный ввод! <b><u>Дата заезда должна быть больше или равна сегодняшней дате!</u></b>"
                        "Повторите выбор даты.",
                        parse_mode="html",
                    )
