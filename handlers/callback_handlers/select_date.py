...
def input_date(call: CallbackQuery) -> None:
    ...
    if action == 'DAY':
        ...
        bot.set_state(call.message.chat.id, User_input_state.input_date)
            ...
            if 'checkInDate' in data:
                ...
                if int(select_date) > checkin:
                    ...
                    if data['sort'] == 'DISTANCE':
                        bot.set_state(call.message.chat.id, User_input_state.land_mark_in)
                        bot.send_message(call.message.chat.id, 'Теперь определимся на каком расстоянии от центра искать отели.\n'
                                                               '<b>Введите <u>начало диапазона</u> расстояния от центра '
                                                               '(<u>от 0 миль</u>).</b>',
                                         parse_mode="html")
                    else:
                        print_data(call.message, data)
                else:
                    bot.send_message(call.message.chat.id, 'Неправильный ввод! <b><u>Дата выезда должна быть больше даты заезда!</u></b>'
                                                           'Повторите выбор даты.', parse_mode='html')
                ...
                if int(select_date) >= int(now):
                    ...
                    bot.send_message(call.message.chat.id, 'Неправильный ввод! <b><u>Дата заезда должна быть больше или равна сегодняшней дате!</u></b>'
                                                           'Повторите выбор даты.', parse_mode='html')
