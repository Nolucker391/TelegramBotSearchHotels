...
def history(message: Message) -> None:
    ...
    bot.send_message(message.chat.id, "<u>Раздел /history (История запросов)</u>. <b>Текущий список:</b> ", parse_mode="html")

    for item in queries:
        bot.send_message(message.chat.id, f"<b>№{item[0]}</b>\n- Дата и время: \t{item[1]}. \n- Вы вводили город: {item[2]}", parse_mode="html")

    bot.set_state(message.chat.id, User_input_state.history_select)

    bot.send_message(message.from_user.id, "<b>Введите номер интересующего вас варианта: </b>", parse_mode="html")
...
def input_city(message: Message) -> None:
    ...
    if message.text.isdigit():
        ...
        if photo_need != 'yes':
            bot.send_message(message.chat.id, '<b>Вы выбирали вариант вывода - "<u>без фото</u>".</b>', parse_mode='html')

        if int(message.text) in number_query:
            history_dict = database.read_from_database.get_history_response(message.text)
            logger.info('Выдаем результаты выборки из базы данных')

            for hotel in history_dict.items():
                medias = []
                caption = f"<b>Название отеля:</b> {hotel[1]['name']}\n<b>Адрес отеля:</b> {hotel[1]['address']}" \
            ...
            bot.send_message(message.chat.id, "<b>Поиск окончен!</b>", parse_mode='html')
