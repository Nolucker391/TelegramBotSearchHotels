...
def low_high_best_handler(message: Message) -> None:
    ...
    database.add_user_in_database.add_user(message.chat.id, message.from_user.username, message.from_user.full_name)
...
def input_city(message: Message) -> None:
    ...
    with bot.retrieve_data(message.chat.id) as data:
        ...
        if response_cities.status_code == 200:
            ...
            bot.send_message(message.chat.id, f"<b>Что-то пошло не так, <u>код ошибки: {response_cities.status_code}</u></b>\n"
                                              f"Нажмите команду еще раз. И введите другой город.", parse_mode='html')
    ...
    try:
        if message.text.isdigit():
            if 0 < int(message.text) <= 10:
                ...
                bot.send_message(message.chat.id, 'Супер! Теперь уточним стоимость отеля за ночь(сутки).'
                                                  '<b>\nВведите <u>минимальную стоимость</u> отеля в долларах $ США:</b>', parse_mode="html")
            else:
                bot.send_message(message.chat.id, 'Неправильный ввод! <b><u>Вводимое число должен быть в диапазоне от 1 до 10!</u></b> Повторите ввод.', parse_mode='html')
        ...
        bot.send_message(message.chat.id, 'Неправильный ввод! <b><u>Вы ввели не число!</u></b> Повторите ввод.', parse_mode='html')
        ...
        if message.text.isdigit():
            ...
            with bot.retrieve_data(message.chat.id) as data:
                if int(data['price_min']) < int(message.text):
                    ...
                    bot.send_message(message.chat.id, 'Неправильный ввод! <b><u>Максимальная цена должна быть больше минимальной!</u></b> Повторите ввод.', parse_mode='html')
    ...
    try:
        if message.text.isdigit():
            if 0 < int(message.text) <= 10:
                ...
                bot.send_message(message.chat.id, 'Неправильный ввод! <b><u>Число фотографий должно быть в диапазоне от 1 до 10!</u></b> Повторите ввод', parse_mode='html')

        elif int(message.text) < 0:
            bot.send_message(message.chat.id, "Неправильный ввод! <b><u>Вводимое число не может быть меньше 0!</u></b> Повторите ввод.", parse_mode='html')
...
def input_landmark_in(message: Message) -> None:
    ...
    try:
        if message.text.isdigit() or float(number):
            ...
            bot.send_message(message.chat.id, '<b>Введите <u>конец диапазона</u> расстояния от центра (<u>в милях</u>).</b>',
                             parse_mode="html")
